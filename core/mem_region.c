...
uint64_t ultra_base __force_data = 0;

unsigned long top_of_ram;

struct mem_region ultra_heap = {
	.name		= "ibm,uv-firmware-heap",
	.start		= 0, /* TBA */
	.len		= 0, /* TBA */
	.type		= REGION_ULTRA_HEAP,
};

static struct mem_region ultra_code_and_text = {
	.name		= "ibm,uv-firmware-code",
	.start		= 0, /* TBA */
	.len		= 0, /* TBA */
	.type		= REGION_ULTRA_FIRMWARE,
};

static struct mem_region ultra_cpu_stacks = {
	.name		= "ibm,uv-firmware-stacks",
	.start		= 0, /* TBA */
	.len		= 0, /* TBA */
	.type		= REGION_ULTRA_FIRMWARE,
};

struct mem_region ultra_stor = {
	.name		= "ibm,uv-stor",
	.start		= 0, /* TBA */
	.len		= 0, /* TBA */
	.type		= REGION_ULTRA_STOR,
};

static struct mem_whitelist *mem_whitelist;
static int nr_mem_wl_resource;
...
static void init_allocatable_region(struct mem_region *region)
{
	...
	assert(region->type == REGION_ULTRA_HEAP ||
	       region->type == REGION_ULTRA_STOR ||
		       ...
		       const char *location)
{
	/* Corrupt: step carefully! */
	if (is_rodata(hdr->location))
		pr_error("%p (in %s) %s at %s, previously %s\n",
			 hdr-1, region->name, during, location, hdr->location);
	else
		pr_error("%p (in %s) %s at %s, previously %p\n",
			 hdr-1, region->name, during, location, hdr->location);
	abort();
}
...
#define for_each_hdr(hdr, region)		\
...
/*
 * When memory allocation fails, we want to show all the active major
 * allocations to help understand memory usage. To avoid flooding the
 * limited UV log space, group allocations by location and size and
 * dump a consolidated "table" of allocations. Consumes some CPU time
 * but would be more useful for debug.
 */
...
static void dump_loc_table(void)
{
	int i;
	int size;
	int total;
	int nentries;
	struct loc_table_entry *entry;
	int small;

	small = total = 0;
	nentries = sizeof(loc_table) / sizeof(struct loc_table_entry);
	...
	for (i = 0; i < nentries; i++) {
		...
		/* count but don't print small/in-frequent allocations */
		size = entry->size * entry->count;
		total += size;
		if (size <= 8192) {
			small++;
			continue;
		...
		pr_error("LOCTBL[%d]: count %d, size %zd (%s)\n", i,
			 entry->count, entry->size, entry->location);
	}

	pr_error("LOCTBL: %d Small allocs (<8K) suppressed)\n", small);
	pr_error("LOCTBL: Total allocated memory %d (%d MB)\n",
		 total, (total >> 20));
}

static void add_loc_table_entry(const char *location, int nlongs)
{
	int i;
	size_t size;
	int nentries;
	...
	for (i = 0; i < nentries; i++) {
		...
		if (entry->location == location && entry->size == size) {
			entry->count++;
			return;
		}
	...
	if (i == nentries) {
		pr_error("All %d loc-table eentries are used?\n", nentries);
		dump_loc_table();
		assert(0);
	}

	loc_table[i].location = location;
	loc_table[i].size = size;
	loc_table[i].count = 1;
}
...
void mem_dump_allocs(void)
{
	...
	memset(&loc_table, 0, sizeof(loc_table));
	pr_info("Memory regions:\n");
	list_for_each(&regions, region, list) {
		if (!(region->type == REGION_ULTRA_HEAP ||
		      region->type == REGION_MEMORY))
			continue;
		pr_info("  0x%012llx..%012llx : %s\n",
		...
		for_each_hdr(hdr, region) {
			if (hdr->free)
				continue;
			add_loc_table_entry(hdr_location(hdr), hdr->num_longs);
		}
	}
	dump_loc_table();
}
#else
#define mem_dump_allocs()
#endif
...
int64_t mem_dump_free(void)
{
	...
	pr_info("Free space in HEAP memory regions:\n");
	list_for_each(&regions, region, list) {
		if (!(region->type == REGION_ULTRA_HEAP ||
		      region->type == REGION_MEMORY))
			continue;
		...
		pr_info("Region %s free: %"PRIu64"\n",
		       region->name, region_free);
		total_free += region_free;
	}

	pr_info("Total free: %"PRIu64"\n", total_free);
		...
		const char *location)
{
	...
	pr_error("mem_alloc(0x%lx, 0x%lx, \"%s\") failed !\n",
		 size, align, location);
	mem_dump_allocs();
#ifdef DUMP_ACTIVE_ALLOCS
	assert(0);
...
bool mem_check(const struct mem_region *region)
{
	...
	for_each_hdr(hdr, region) {
		if (hdr->num_longs < ALLOC_MIN_LONGS) {
			pr_error("Region '%s' %s %p (%s) size %zu\n",
		...
		if ((unsigned long)hdr + hdr->num_longs * sizeof(long) >
			...
			pr_error("Region '%s' %s %p (%s) oversize %zu\n",
				 region->name, hdr->free ? "free" : "alloc",
				 hdr, hdr_location(hdr),
				 hdr->num_longs * sizeof(long));
			return false;
		...
		if (hdr->free) {
			if (hdr->prev_free || prev_free) {
				pr_error("Region '%s' free %p (%s) has "
					 "prev_free %p (%s) %sset?\n",
			...
			if (hdr->prev_free != (bool)prev_free) {
				pr_error("Region '%s' alloc %p (%s) has"
					 " prev_free %p %sset?\n",
					 region->name, hdr, hdr_location(hdr),
	...
	if (frees) {
		pr_error("Region '%s' free list and walk do not match!\n",
			 region->name);
		return false;
	}
	return true;
...
static bool add_region(struct mem_region *region)
{
	...
	if (mem_regions_finalised) {
		pr_error("MEM: add_region(%s@0x%"PRIx64") "
			 "called after finalise!\n",
	...
	list_for_each(&regions, r, list) {
		...
		if (overlaps(r, region) && !contains(r, region)) {
			pr_error("MEM: Partial overlap detected between regions:\n");
			pr_error("MEM: %s [0x%"PRIx64"-0x%"PRIx64"] (new)\n",
				 region->name, region->start,
				 region->start + region->len);
			pr_error("MEM: %s [0x%"PRIx64"-0x%"PRIx64"]\n",
	...
	pr_info("%s region start=%llx size=%llx\n",
		region->name, region->start, region->len);
	return true;
}
...
{
	/** @todo (andmike) Should top_of_ram init move */
	top_of_ram = ultra_base + ULTRA_SIZE;

	ultra_code_and_text.start = ultra_base;
	ultra_code_and_text.len = TEXT_AND_BSS_SIZE;

	ultra_heap.start = ultra_base + HEAP_OFFSET;
	ultra_heap.len = HEAP_SIZE;

	ultra_stor.start = ultra_base + UV_STOR_OFFSET;
	ultra_stor.len = UV_STOR_SIZE;
}

/**
 * @brief Wrapper to add page alloc ranges.
 */
static void mem_region_page_alloc_add(int32_t node_id, u64 start, u64 len)
{
	u64 end;

	end = start + len;

	if (ultra_base >= start && ultra_base < end) {
		u64 _used;
		/* Adjust range for space used by uv firmare */
		pr_info("MEM: uv-firmware ultra_base %" PRIx64 "\n",
			ultra_base);
		pr_info("MEM: uv-firmware start %" PRIx64 "\n",
			start);
		pr_info("MEM: uv-firmware len %" PRIx64 "\n", len);
		_used = top_of_ram - start;
		pr_info("MEM: used %" PRIx64 "\n", _used);
		start = ((top_of_ram + UV_PAGE_SIZE - 1) & UV_PAGE_MASK);
		_used += start - top_of_ram;
		pr_info("MEM: Pg Size Round Up start  %" PRIx64 "\n",
			start);
		len = len - _used;
		pr_info("MEM: Adjusting len %" PRIx64 "\n", len);
	}

	pr_info("MEM: NUMA Node %d (start=%" PRIx64 ", len %" PRIx64 ")\n",
		node_id, start, len);
	add_numa_node(node_id, (void *) start, len);
}

static void mem_register_secure_region(void)
{
	struct mem_region *r;

	list_for_each(&regions, r, list) {
		if (r->type != REGION_SECURE_MEM)
			continue;

		mem_region_page_alloc_add(r->node_id, r->start, r->len);
	}
}

static int mem_region_parse_secure_memory(void)
{
	struct dt_node *smem;
	struct mem_region *region;
	int found = 0;

	/* Find every secure-memory node to call init_numa */
	dt_for_each_compatible(dt_root, smem, "ibm,secure-memory")
		found ++;
	if (!found)
		return 0;
	...
	dt_for_each_compatible(dt_root, smem, "ibm,secure-memory") {
		...
		reg = dt_find_property(smem, "reg");
		if (!reg) {
			pr_error("BAD secure-memory node %s",
				 smem->name);
			continue;
		}

		start = ((u64)dt_property_get_cell(reg, 0) << 32) |
			dt_property_get_cell(reg, 1);
		size = ((u64)dt_property_get_cell(reg, 2) << 32) |
			dt_property_get_cell(reg, 3);

		region = new_region("ibm,secure-memory", start, size,
				    smem, REGION_SECURE_MEM);
		assert(region);

		region->node_id = numa_associativity_to_nid(smem);

		pr_info(
		      "MEM: secure memory [0x%"PRIx64"-0x%"PRIx64"] node: %d\n",
		      start, start + size, region->node_id);

		if (!add_region(region)) {
			pr_error("Couldn't add secure memory "
				 "[0x%"PRIx64"-0x%"PRIx64"\n",
				 start, start + size);
			abort();
		}
...
static void mem_region_parse_reserved_properties(void)
{
	...
	pr_debug("MEM: parsing reserved memory from "
	...
	if (names && ranges) {
		...
		for (n = 0; n < names->len; n += len, range += 2) {
			...
			if (!add_region(region)) {
				pr_error("Couldn't add mem_region %s\n", name);
				abort();
			}
		}
	} else if (names || ranges) {
		pr_error("Invalid properties: reserved-names=%p "
...
static bool mem_region_parse_reserved_nodes(const char *path)
{
	...
	pr_info("MEM: parsing reserved memory from node %s\n", path);
	...
	dt_for_each_child(parent, node) {
		...
		if (!reg) {
			...
			pr_error("node %s has no reg property, ignoring\n",
				 nodepath);
			free(nodepath);
			continue;
		}
		...
		region = new_region(node->name,
				dt_get_number(reg->prop, 2),
				dt_get_number(reg->prop + sizeof(u64), 2),
				node, type);
		if (!add_region(region)) {
			char *nodepath = dt_get_path(node);
			pr_error("node %s failed to add_region()\n", nodepath);
			free(nodepath);
		}
	}
...
{
	int rc;
	/* @todo: Ensure we have no collision between ultra core and our heap */
	extern char _end[];

	pr_debug("MEM: Enter %s w/ end %" PRIx64 " top of ram 0x%lx\n",
		 __func__, (uint64_t)_end, top_of_ram);
	/*
	 * This is called after we know the maximum PIR of all CPUs,
	 * so we can dynamically set the stack length.
	 */
	ultra_cpu_stacks.start = ultra_base + ULTRA_SIZE;
	ultra_cpu_stacks.len = (cpu_max_pir + 1) * STACK_SIZE;

	top_of_ram += ultra_cpu_stacks.len;
	pr_debug("MEM: (%d) top_of_ram 0x%lx \n", __LINE__, top_of_ram);

	lock(&mem_region_lock);

	mem_region_parse_secure_memory();

	if (!add_region(&ultra_code_and_text)) {
		abort();
	}
	if (!add_region(&ultra_heap)) {
		abort();
	}
	if (!add_region(&ultra_cpu_stacks)) {
		abort();
	}
	if (!add_region(&ultra_stor)) {
		abort();
	}
	...
	list_for_each(&regions, r, list) {
		...
		/* Partially used?  Split region. */
			...
			for_linux = split_region(r, r->start + used_len,
						 REGION_OS);
			if (!for_linux) {
				pr_error("OOM splitting mem node %s for linux\n",
					 r->name);
				abort();
			}
		...
		struct mem_region *region)
{
	...
	/*
	 * If a reserved region was established before ultra, it may be
...
/*
 * return the whitelist memory resource with rid @rid and type @rtype.
 */
static struct mem_whitelist *get_mem_wl(u32 rtype, u32 rid)
{
	int i;

	for (i = 0; i < nr_mem_wl_resource; i++)
		if (mem_whitelist[i].rtype == rtype &&
		    mem_whitelist[i].rid == rid)
			return &mem_whitelist[i];

	return NULL;
}

struct mem_whitelist *mem_wl_range_allowed(u32 rtype, u32 rid,
	       u64 offset, u32 size, bool is_write)
{
	int i;
	u64 r_start = offset;
	u64 r_end = offset + size - 1;
	struct mem_whitelist *mem = get_mem_wl(rtype, rid);
	struct white_list *access = mem->read;
	int nr_list = mem->nr_read_list;
	...
	if (is_write) {
		access = mem->write;
		nr_list = mem->nr_write_list;
	}

	for (i = 0; i < nr_list; i++) {
		u64 start = access[i].offset;
		u64 end = start + access[i].size - 1;
...
/*
 * initialize the memory resource whitelist with len items.
 */
static void init_mem_wl_resource(const struct dt_property *resource, int len,
		struct white_list **wl)
{
	int i, j, k;
	u64 offset, size;

	*wl = malloc(sizeof(*wl) * len);
	assert(wl);
	/* keep the list sorted. Saves time while searching */
	for (j = 0; j < len; j++) {
		k = j * 4;
		offset = ((u64)dt_property_get_cell(resource, k) << 32) |
				dt_property_get_cell(resource, k + 1);
		size = ((u64)dt_property_get_cell(resource, k + 2) << 32) |
				dt_property_get_cell(resource, k + 3);

		for (i = j-1 ; i >= 0; i--) {
			if ((*wl)[i].offset < offset)
				break;
			(*wl)[i+1] =  (*wl)[i];
		}
		(*wl)[i+1].offset =  offset;
		(*wl)[i+1].size =  size;
	}
}

/*
 * initialize all memory resource whitelist.
 */
void init_mem_whitelist(void)
{
	struct dt_node *mem_node, *node;
	int nr_regions = 0, i = 0;

	mem_node = dt_find_by_path(dt_root, "/ibm,opal/power-mgt/mem_range");
	if (!mem_node)
		return;

	dt_for_each_node(mem_node, node)
		nr_regions++;

	nr_mem_wl_resource = nr_regions;
	mem_whitelist = malloc(sizeof(struct mem_whitelist) * nr_regions);
	assert(mem_whitelist);
	...
	dt_for_each_node(mem_node, node) {
		...
		if (!reg)
			continue;
		mem_whitelist[i].start =
			((u64)dt_property_get_cell(reg, 0) << 32) |
			dt_property_get_cell(reg, 1);
		mem_whitelist[i].size =
			((u64)dt_property_get_cell(reg, 2) << 32) |
			dt_property_get_cell(reg, 3);

		pr_info("MEM RSRC start = %016llx %016llx\n",
			mem_whitelist[i].start, mem_whitelist[i].size);

		mem_whitelist[i].rid = dt_prop_get_u32_def(node,
							   "ibm,chip-id", 0);
		mem_whitelist[i].rtype = dt_prop_get_u32(node,
							 "ibm,resource-type");

		reg = dt_find_property(node, "ibm,write-offsets");
		if (reg)
			init_mem_wl_resource(reg,
					(int)(reg->len / (sizeof(u32) * 4)),
					&mem_whitelist[i].write);

		reg = dt_find_property(node, "ibm,read-offsets");
		if (reg)
			init_mem_wl_resource(reg,
					(int)(reg->len / (sizeof(u32) * 4)),
					&mem_whitelist[i].read);
		i++;
	}
}
