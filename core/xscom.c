...
static inline void *xscom_addr(uint32_t gcid, uint32_t pcb_addr)
{
	...
	/* Enable bit[0] of the address to bypass the urmor update */
	addr |= (1ULL << 63);
...
static void xscom_reset(uint32_t gcid, bool need_delay)
{
 ...
 fail:
	...
	pr_error("XSCOM: Fatal error resetting engine after failed access !\n");
...
static int64_t xscom_handle_error(uint64_t hmer, uint32_t gcid,
				  uint32_t pcb_addr, bool is_write,
				  int64_t retries, int64_t *xscom_clear_retries)
{
	unsigned int stat = GETFIELD(SPR_HMER_XSCOM_STATUS, hmer);
	int64_t rc = U_HARDWARE;
	...
	case 1:
		...
		if (retries && !(retries  % XSCOM_BUSY_RESET_THRESHOLD)) {
			pr_notice("XSCOM: Busy even after %d retries, "
		...
		if (retries == XSCOM_BUSY_MAX_RETRIES)
			pr_error("XSCOM: %s-busy error gcid=0x%x pcb_addr=0x%x "
				 "stat=0x%x\n", is_write ? "write" : "read",
				 gcid, pcb_addr, stat);
		return U_XSCOM_BUSY;
	...
	case 2: /* CPU is asleep, reset XSCOM engine and return */
		...
		return U_XSCOM_CHIPLET_OFF;
	case 3: /* Partial good */
		rc = U_XSCOM_PARTIAL_GOOD;
		break;
	case 4: /* Invalid address / address error */
		rc = U_XSCOM_ADDR_ERROR;
		if (xscom_clear_error(gcid, pcb_addr)) {
			/* return busy if retries still pending. */
			if ((*xscom_clear_retries)--)
				return U_XSCOM_BUSY;

			pr_debug("XSCOM: error recovery failed for "
				 "gcid=0x%x pcb_addr=0x%x\n", gcid, pcb_addr);

		}
		break;
	case 5: /* Clock error */
		rc = U_XSCOM_CLOCK_ERROR;
		break;
	case 6: /* Parity error  */
		rc = U_XSCOM_PARITY_ERROR;
		break;
	case 7: /* Time out */
		rc = U_XSCOM_TIMEOUT;
		break;
	...
	pr_error("XSCOM: %s error gcid=0x%x pcb_addr=0x%x stat=0x%x\n",
				   ...
				   uint64_t pcb_addr, bool is_write)
{
	...
	/* XXX: Create error log entry ? */
	if (timeout)
		pr_error("XSCOM: indirect %s timeout, gcid=0x%x pcb_addr=0x%llx"
			 " stat=0x%x\n",
			 is_write ? "write" : "read", gcid, pcb_addr, stat);
	else
		pr_error("XSCOM: indirect %s error, gcid=0x%x pcb_addr=0x%llx"
			 " stat=0x%x\n",
			 is_write ? "write" : "read", gcid, pcb_addr, stat);
}
...
static int __xscom_read(uint32_t gcid, uint32_t pcb_addr, uint64_t *val)
{
	...
	if (!xscom_gcid_ok(gcid)) {
		pr_error("%s: invalid XSCOM gcid 0x%x\n", __func__, gcid);
		return U_PARAMETER;
	}
	...
	if (xscom_is_multicast_addr(pcb_addr) && ret == U_XSCOM_CHIPLET_OFF)
		return ret;
	 ...
	 */
	if (proc_gen == proc_gen_p9 && ret == U_XSCOM_CHIPLET_OFF)
		return ret;

	pr_error("XSCOM: Read failed, ret = %lld\n", ret);
	return ret;
}
...
static int __xscom_write(uint32_t gcid, uint32_t pcb_addr, uint64_t val)
{
	...
	pr_error("XSCOM: Write failed, ret = %lld\n", ret);
	return ret;
}
				      ...
				      uint64_t val)
{
	...
	for (retries = 0; retries < XSCOM_IND_MAX_RETRIES; retries++) {
		...
		if ((data & XSCOM_DATA_IND_COMPLETE) ||
		    (retries >= XSCOM_IND_MAX_RETRIES)) {
			xscom_handle_ind_error(data, gcid, pcb_addr, true);
			rc = U_HARDWARE;
			goto bail;
		}
	}
		...
		bool take_lock)
{
	...
	/*
	 * Rather than a random value let's thus
	...
	*val = 0xdeadbeefdeadbeefull;
	...
	default:
		 ...
		 */
		pr_error("%s: invalid XSCOM partid 0x%x\n", __func__, partid);
		return U_PARAMETER;
	}
...
int _xscom_write(uint32_t partid, uint64_t pcb_addr, uint64_t val,
		 bool take_lock)
{
	uint32_t gcid;
	int rc;
...
static void xscom_init_chip_info(struct proc_chip *chip)
{
	...
	if (rc) {
		pr_error("XSCOM: Error %lld reading 0xf000f register\n", rc);
	...
	case 0xd4:
		...
		break;
	default:
		pr_warn("CHIP: Unknown chip type 0x%02x !!!\n",
	...
	if (chip_quirk(QUIRK_MAMBO_CALLOUTS)) {
		...
		pr_notice("P9 DD%i.%i%d detected\n",
			  0xf & (chip->ec_level >> 4),
...
void xscom_init(void)
{
	...
	dt_for_each_compatible(dt_root, xn, "ibm,xscom") {
		...
		pr_notice("CHIP: Chip ID %04x type: %s DD%x.%x\n",
			  gcid, chip_name, chip->ec_level >> 4,
			  chip->ec_level & 0xf);
		pr_debug("XSCOM: Base address: 0x%llx\n", chip->xscom_base);
	}
	...
	if (p) {
		...
		pr_debug("XSTOP: XSCOM addr = 0x%llx, FIR bit = %lld\n",
			 xstop_xscom.addr, xstop_xscom.fir_bit);
	} else
		pr_debug("XSTOP: ibm,sw-checkstop-fir prop not found\n");
}
