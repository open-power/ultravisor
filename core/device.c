					      ...
					      const char *name, int wanted_len)
{
	...
	if (!p) {
		...
		pr_error("DT: Missing required property %s/%s\n",
			 path, name);
		assert(false);
	}
	if (wanted_len >= 0 && p->len != wanted_len) {
		...
		pr_error("DT: Unexpected property length %s/%s\n",
			 path, name);
		pr_error("DT: Expected len: %d got len: %zu\n",
			 wanted_len, p->len);
		assert(false);
	}
	...
	do {
		...
		case FDT_BEGIN_NODE:
			...
			if (!dt_attach_root(node, child))
				 ...
				 * functional.
	                         */
				pr_error("DT: Found duplicate node: %s\n",
					 child->name);
			break;
		case FDT_END:
			return -1;
