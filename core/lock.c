...
static void __nomcount lock_error(struct lock *l, const char *reason)
{
...
static inline void __nomcount unlock_check(struct lock *l)
{
	...
	if (l->in_con_path && this_cpu()->con_suspend == 0)
		lock_error(l, "Unlock con lock with console not suspended");

	if (list_empty(&this_cpu()->locks_held))
		lock_error(l, "Releasing lock we don't hold depth");
}
...
static void add_lock_request(struct lock *l)
{
	...
	if (dead)
		lock_error(l, "Deadlock detected");
}
...
void lock_caller(struct lock *l, const char *owner)
{
	...
	for (;;) {
		...
		if (start && !timeout_warn && lock_timeout(start)) {
			 ...
			 */
			remove_lock_request();
			pr_warn("Lock has been spinning for over %dms\n",
				LOCK_TIMEOUT_MS);
			backtrace();
			add_lock_request(l);
			timeout_warn = true;
