...
.. code-block:: c
        ...
        uint64_t ucall(const uint64_t UV_WRITE_SCOM,
                       uint64_t partid,
                       uint64_t pcb_addr,
                       uint64_t val)
...
~~~~~~~~~~~
    ...
    if write-operation to the specified XSCOM address is disallowed for the
    requestor; as specified in the whitelist/blacklist, the write-operation is
    failed.

Use cases
~~~~~~~~~

    Skiboot needs to write some XSCOM addresses to operate correctly. These
    XSCOM addresses are monitored by ultravisor through whitelist/blacklist. 
    If Skiboot requests write to addresses that are disallowed, ultravisor
    fails the operation. Skiboot cannot access the XSCOM addresses directly
    when SMF/PEF is enabled.
...
.. code-block:: c
        ...
        uint64_t ucall(const uint64_t UV_READ_SCOM,
                   uint64_t long *retbuf,
                   uint64_t partid,
                   uint64_t pcb_addr)
...
~~~~~~~~~~~
    ...
    if read-operation to the specified XSCOM address is disallowed for the
    requestor; as specified in the whitelist/blacklist, the read-operation is
    failed.

Use case
~~~~~~~~~

    #. Skiboot needs to read some XSCOM addresses to operate correctly. These
    XSCOM addresses are monitored by ultravisor through whitelist/blacklist. 
    If the Skiboot requests read-operation to a disallowed address, ultravisor
    fails the operation. Skiboot cannot access the XSCOM address directly,
    when SMF/PEF is enabled.
...
UV_READ_MEM
-------------
    Read the content at a well-known memory location.
...
.. code-block:: c
	...
	uint64_t ultracall(const uint64_t UV_READ_MEM,
		uint64_t buffer, /* buffer to place the memory content */
		uint64_t handle, /* the handle id of the memory location */
		uint64_t offset, /* offset within the memory location pointed by
                                    handle */
...
~~~~~~~~~~~~~
	...
	* U_SUCCESS	on success.
	* U_FUNCTION	if functionality is not supported.
	* U_PARAMETER	if any parameter is invalid
	* U_PERMISSION	if the operation is not allowed.

Description
~~~~~~~~~~~

    if read-operation to the specified memory location is disallowed for the
    requestor; as specified in the whitelist/blacklist for memory handles,
    the read-operation is failed.

Use cases
~~~~~~~~~

    #. Skiboot needs to read some secure memory location to operate correctly. 
    These are well-known memory locations that are pre-defined through handles.
    If the requested handle is disallowed to be read by less privileged software,
    ultravisor fails the operation.
...
UV_WRITE_MEM
-------------
    Write the content at the well-known memory location.
...
.. code-block:: c
	...
	uint64_t ultracall(const uint64_t UV_WRITE_MEM,
		uint64_t buffer, /* buffer to place the memory content */
		uint64_t handle, /* the handle id of the memory location */
		uint64_t offset, /* offset within the memory location pointed by
                                    handle */
...
~~~~~~~~~~~
    ...
    if write-operation to the specified memory location is disallowed for the
    requestor; as specified in the whitelist/blacklist for memory handles,
    the write-operation is failed.

Use cases
~~~~~~~~~

    #. Skiboot needs to write some secure memory location to operate correctly. 
    These are well-known memory locations that are pre-defined through handles.
    If the requested handle is disallowed to be write by less privileged software,
    ultravisor fails the operation.
...
-------------------
    ...
    Execute the specified SBE command.
...
.. code-block:: c
	...
	uint64_t ultracall(const uint64_t UV_SEND_SBE_COMMAND,
		uint64_t chip_id,/* the chip id targeted for the command */
		uint64_t opcode, /* the opcode of the command */
		uint64_t input,  /* input buffer address */
		uint32_t output  /* output buffer address */
...
~~~~~~~~~~~
    ...
    Execute the command specified in the opcode, on the specified chip id.
    Use the input from the input buffer and place the output in the output
    buffer. Currently the following opcodes are supported.
    SBE_CHIPOP_TIMER, SBE_CHIPOP_HANDLE_INTERRUPT and SBE_CHIPOP_START_MPIPL.

Use cases
~~~~~~~~~

    #. Skiboot cannot directly interact with SBE when PEF/SMF is enabled.
    This interface allows Skiboot to execute specific set of chipop operations
    with the help of the ultravisor.
