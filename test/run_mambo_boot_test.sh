...
#    case "$1" in
#      -d|--datestamp)
#        [[ $# -lt 2 ]] && stderr_exit "Missing value for ${1}" 1
#        script_date="${2}"
#        shift
#        ;;
#      -h|--help)
...
#  done
#
#  script_date=$(date -u -d @"${script_date}" +%Y%m%dT%H%M%SZ)
#
...
#  fi
#
#  run_test "${build_dir}"/external/mambo/ "${tcl_file}"
#
  exit 0
}
...
write_stdout "Mambo test currently disabled"
exit 0
